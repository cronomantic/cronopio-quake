...
/* [cronopio] R_AccelDrawing is defined by the platform/cart (r_accel_cron.c).
 * It was declared in render.h on the fork branch; this compat override keeps the
 * upstream headers pure, so declare it locally instead. */
...
void R_ViewChanged(vrect_t* pvrect, i32 lineadj, float aspect) {
    ...
    xcenter =
        ((float) r_refdef.vrect.width * XCENTERING) + r_refdef.vrect.x - 0.5;
    aliasxcenter = xcenter * r_aliasuvscale;
    ycenter =
        ((float) r_refdef.vrect.height * YCENTERING) + r_refdef.vrect.y - 0.5;
        ...
        switch (currententity->model->type) {
            case mod_brush:
                ...
                for (j = 0; j < 3; j++) {
                    ...
                    minmaxs[3 + j] =
                        currententity->origin[j] + clmodel->maxs[j];
                }
...
void R_EdgeDrawing(void) {
    ...
    if (auxedges) {
        ...
        r_edges = (edge_t*) (((intptr_t) &ledges[0] + CACHE_SIZE - 1) &
                             ~(CACHE_SIZE - 1));
    }

    if (r_surfsonstack) {
        surfaces = (surf_t*) (((intptr_t) &lsurfs[0] + CACHE_SIZE - 1) &
                              ~(CACHE_SIZE - 1));
        surf_max = &surfaces[r_cnumsurfs];
        // surface 0 doesn't really exist; it's just a dummy because index 0
        // is used to indicate no edge attached to surface
